...
void handleIRC(IRCMessage message, IRCClient *client)
{
    ...
    if (cmd == "PRIVMSG")
    {
        ...
        if (channel == manager.irc->getData().comms_channel)
        {
            if (msg.find("msg") == 0)
            {
                w->addToStringList(QString::fromStdString(usr),
                                   QString::fromStdString(msg.substr(3)));
                return;
            }
        }
    }
...
void MainWindow::on_reconnectButton_clicked()
{
    ...
    manager.irc->UpdateData(user, user, "#cat_comms", channel, password,
                            address, port);
    manager.irc->Connect();
}
...
void MainWindow::on_cmdLineEdit_returnPressed()
{
    if (ui->sendallBox->checkState())
        manager.irc->privmsg(
            "cc_cmd$cmd" + ui->cmdLineEdit->text().toStdString(), true);
    else
    {
        ...
        manager.irc->privmsg("cc_cmd$id" + std::to_string(id) + "$cmd" +
                                 ui->cmdLineEdit->text().toStdString(),
                             true);
    }
